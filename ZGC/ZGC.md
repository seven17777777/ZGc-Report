...
### 4.2 垃圾回收过程中的低暂停时间：
ZGC的主要优势是其低暂停时间。目标是将垃圾回收的暂停时间保持在10毫秒以内，且与堆的大小无关。这一点在后面的GC日志中随处可见。修改程序为一次配置1m，且堆内存为16m
得到日志([Xmx16m](Xmx/Xmx16m.log))
对日志进行分析：最大暂停时间并没有超过10ms，说明在极小堆的情况下，也能让一gc时间小于10ms，且gc时间和堆大小关系不大。
...
### 4.4 Zgc已经摒弃分代收集
ZGC是一种面向低延迟的垃圾回收器，它采用了单代架构。与经典的多代垃圾回收策略（如CMS或G1）不同，ZGC没有明确区分年轻代和老年代。
所有对象在ZGC中都被视为同一代，并且通过标记-清除（Mark-Compact）的方式进行垃圾回收。<br>
可以清楚的看到：日志中并没有关于老年代和年轻代的区分。<br>
另外，在java21中使用-XX:+UseZGC -XX:+ZGenerational ZGC 将会将堆内存划分为年轻代和老年代，并分别进行垃圾回收，从而优化内存管理和提高应用性能。
...
上面考虑的是堆内存为2g的情况，本来进行的GC次数就不会太多，所有造成的效果比较好，如果考虑到128m下，情况应该不会有上面变化
于是下面进行了128m的实验得到结果如下
